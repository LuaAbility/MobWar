function main(abilityData)
	...
	plugin.addPassiveScript(abilityData, 1, function(p)
		...
		if game.getPlayer(p):getVariable("MW007-redrum") == "true" then p:addPotionEffect(newInstance("$.potion.PotionEffect", {effect.INCREASE_DAMAGE, 20, 0})) end
	end)
	
	plugin.addPassiveScript(abilityData, 600, function(p)
		if game.getPlayer(p):getVariable("MW007-redrum") == "true" then p:addPotionEffect(newInstance("$.potion.PotionEffect", {effect.CONFUSION, 300, 0})) end
	end)
