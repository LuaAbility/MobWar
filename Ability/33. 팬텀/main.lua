function main(abilityData)
	...
	plugin.addPassiveScript(abilityData, 200, function(p)
		phantom(p)
		local targetPlayer = game.getPlayer(p):getVariable("MW033-targetPlayer")
		print(targetPlayer)
		if targetPlayer ~= "" then 
			local players = util.getTableFromList(game.getPlayers())
			for i = 1, #players do
				if players[i]:getPlayer():getName() == targetPlayer then phantom(players[i]:getPlayer()) end
			end
		end
	end)
		...
		if e:getDamager():getType():toString() == "PLAYER" and e:getEntity():getType():toString() == "PLAYER" then
			...
			if game.isAbilityItem(item[1], "IRON_INGOT") then
				if game.checkCooldown(e:getDamager(), a, 0) then
					game.getPlayer(e:getDamager()):setVariable("MW033-targetPlayer", e:getEntity():getName())
					...
					util.runLater(function() 
						...
						game.getPlayer(e:getDamager()):removeVariable("MW033-targetPlayer")
					end, 24000)
				end
			end
		end
