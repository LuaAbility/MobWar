...
function onEvent(funcTable)
	if funcTable[1] == "MW022-shoot" and funcTable[2]:getEventName() == "EntityDamageByEntityEvent" then shoot(funcTable[2], funcTable[4], funcTable[1]) end
	if funcTable[1] == "MW022-cancelTarget" and funcTable[2]:getEventName() == "EntityTargetLivingEntityEvent" then cancelTarget(funcTable[2], funcTable[4], funcTable[1]) end
end
...
function shoot(event, ability, id)
	if event:getDamager():getType():toString() == "PLAYER" and event:getEntity():getType():toString() == "PLAYER" then
		...
		if game.isAbilityItem(item, "IRON_INGOT") then
			if game.checkCooldown(game.getPlayer(event:getDamager()), ability, id) then
				...
				event:getEntity():addPotionEffect(newInstance("$.potion.PotionEffect", {effect.POISON, 200, 1}))
